...
impl DaemonClient {
    /// Создает новый экземпляр клиента, используя глобально кешированный `reqwest::Client`.
    /// Это позволяет переиспользовать пул соединений и избегать затрат на создание клиента
    /// при частых вызовах API (например, при поллинге статуса).
    pub fn new() -> Self {
