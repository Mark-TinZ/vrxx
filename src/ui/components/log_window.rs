    ...
    fn load_logs_from_file(&self) {
        ...
        if let Ok(mut file) = std::fs::File::open(log_path) {
            ...
            if file.seek(SeekFrom::Start(start_pos)).is_ok() {
                ...
                // OPTIMIZE: Делаем один скролл после загрузки всех логов
                // чтобы избежать O(n²) layout thrashing при массовой вставке
                if self.imp().btn_autoscroll.is_active() {
                    let buffer = self.imp().text_view.buffer();
                    let mark = buffer.create_mark(None, &buffer.end_iter(), false);
                    self.imp().text_view.scroll_to_mark(&mark, 0.0, false, 0.0, 1.0);
                    buffer.delete_mark(&mark);
                }
            }
        }
