...
fn get_singbox_version() -> (u32, u32, u32) {
    if let Ok(output) = std::process::Command::new("sing-box")
        .arg("version")
        .output()
    {
...
mod tests {
    ...
    fn test_singbox_ipv6_disabling() {
        ...
        let has_ipv6_block = rules.iter().any(|r| {
            r["ip_cidr"]
                .as_array()
                .map_or(false, |cidrs| cidrs.contains(&json!("::/0")))
        ...
        if is_1_12_or_newer {
            ...
            let has_aaaa_reject = dns_rules.iter().any(|r| {
                r["query_type"]
                    .as_array()
                    .map_or(false, |qt| qt.contains(&json!("AAAA")))
    ...
    fn test_singbox_config_validity_permutations() {
        ...
        for (ipv6, ru, cn) in combinations {
            ...
            let output = child
                .wait_with_output()
                .expect("Failed to wait on sing-box check");
            let stderr = String::from_utf8_lossy(&output.stderr);

            assert!(
                output.status.success(),
                "Sing-box check failed for toggles (IPv6: {}, RU: {}, CN: {}):\n{}",
                ipv6,
                ru,
                cn,
                stderr
