...
## 2024-05-22 - [Visual data leakage in UI]
**Vulnerability:** The UUID/Password input field in the VPN configuration page used a standard `adw::EntryRow`, exposing sensitive credentials visually on the screen.
**Learning:** When displaying or accepting sensitive data (like passwords, tokens, or UUIDs) in a GTK4/Libadwaita application, standard entry rows lack visual masking, leading to potential shoulder surfing vulnerabilities or exposure during screen sharing.
**Prevention:** Always use `adw::PasswordEntryRow` instead of `adw::EntryRow` for sensitive data fields to ensure the text is masked by default while retaining the ability to copy or edit it securely.
