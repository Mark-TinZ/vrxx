...
## 2026-05-19 - Cache HTTP Client in Polling Workers
**Learning:** Recreating `reqwest::Client` repeatedly during frequent polling loops introduces significant overhead because it instantiates a new HTTP connection pool and TLS context each time. In `DaemonClient`, recreating the client on every initialization was wasting resources.
**Action:** Use `std::sync::OnceLock` to lazily initialize `reqwest::Client` globally and clone it into `DaemonClient` instances. Since `reqwest::Client` internally shares its pool via `Arc`, cloning is cheap and preserves Keep-Alive connections.
