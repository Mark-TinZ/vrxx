...
## 2026-05-24 - Optimize DaemonClient HTTP Connection Pooling
**Learning:** Creating `reqwest::Client` holds an internal connection pool and workers. Creating it on every `DaemonClient::new()` call causes significant performance degradation due to re-initialization and dropped keep-alive connections.
**Action:** Cache the `reqwest::Client` in a global `OnceLock` and clone it on demand, which is extremely cheap (clones an `Arc` underneath) and preserves the underlying connection pool.
