...
## 2026-05-11 - Cache reqwest::Client

**Learning:** Recreating `reqwest::Client` for every API call introduces unnecessary overhead due to internal connection pool and DNS resolver initialization.
**Action:** Use `std::sync::OnceLock` (or `lazy_static`/`OnceCell`) to cache the `reqwest::Client` globally, and clone it when needed, as it internally uses `Arc` and is cheap to clone.
