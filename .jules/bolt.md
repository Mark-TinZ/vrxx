...
## 2024-05-30 - Optimize REST API Client Caching
**Learning:** Recreating `reqwest::Client` for every REST API call (like `is_running` polling or starting/stopping the proxy) introduces significant initialization overhead due to TLS contexts and connection pool setup.
**Action:** The REST API client (`reqwest::Client`) can be cached entirely via `std::sync::OnceLock` in `src/ipc.rs`, completely eliminating repeated creation overhead for standard REST calls. Since `reqwest::Client` uses an internal `Arc`, cloning it is cheap and correctly shares the connection pool.
