...
## 2024-05-30 - Cache REST API client for DaemonClient
**Learning:** Recreating `reqwest::Client::new()` in polling or hot paths introduces noticeable overhead and fails to reuse the internal connection pool.
**Action:** Use a static `std::sync::OnceLock<reqwest::Client>` to globally cache the HTTP client for REST APIs, enabling connection keep-alive and lower latency during frequent UI polls.
