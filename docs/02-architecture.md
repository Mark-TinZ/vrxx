...
1. **Слой UI (GTK4/Libadwaita)** — некомпонентный фронтенд, работающий с обычными правами пользователя.
...
- **4 основные страницы**: VPN (главная), Прокси, Белый список, Настройки. Переключение осуществляется через боковую панель.
